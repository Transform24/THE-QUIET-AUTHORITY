...
- **Approval gate (FIXED 2026-06-06)** — all agents save drafts to workflows/output/[platform]-pending/ on main. Sanctuary Approval Gate (approval-gate.html) reads pending items, Grace clicks APPROVE/REJECT to move to -approved/ folder. Deploy workflows auto-post from -approved/. Workflow fixes: .gitignore cache cleared + `git add -f` + force push + commit to main (not side branches).
- **Pinterest Agent fully operational (2026-06-06)** — End-to-end working: generates daily pin caption → outputs to workflows/output/pinterest-pending/ → commits to main → visible in Approval Gate → Grace approves → moves to -approved/ → deploy posts to Pinterest API.
