...
- **Buyer tagging** — was Stripe → Systeme.io, direct (no Make.com). Systeme.io is now shut down; this pipeline stage has no live destination. See `_system/status.md` for the still-running `.github/workflows/gate-buyer-sync.yml` cron that needs a decision (disable, or repoint to a new CRM).
