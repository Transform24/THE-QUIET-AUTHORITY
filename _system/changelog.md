...
- **2026-09-24 — About replaces TQA assessment at repo root.** `about.html` renamed to `index.html` (canonical/og:url updated to `/`); the former `index.html` (TQA's 8-question assessment) renamed to `discover-your-profile.html` (canonical/og:url updated to `/discover-your-profile.html`), internal app logic untouched. Six links repointed from `index.html` to `discover-your-profile.html` across `the-quiet-authority-foyer.html`, `library.html` (×2), `names-of-god.html` (×2), and `foyer.html`, preserving existing `?view=shop`/`?view=silence` query params. Verified locally with a static server + Playwright: root renders About, the assessment works unchanged at its new URL, all six repointed links and About's two CTAs (Gate Zero, Secret Place) resolve correctly. See `_system/status.md` for the full note, including a pre-existing unrelated dead-code flag (`addToSysteme` calls with no definition) found during verification but left untouched, out of scope.
